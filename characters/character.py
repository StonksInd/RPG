...
class Character:
    ...
    def magic_dammage_reduction(self, magic_dammage, niveau_de_sort, other) -> int:
        magic_resistance = self.armor.magic_resistance
        crit_esquive = randint(0,101)
        
        if crit_esquive <= 5:
            print("vous avez fait un critique")
            self.hp_basic -= ((1-(magic_resistance/500)) * ((magic_dammage *niveau_de_sort)* 1.5))
            other.mana_basic -= niveau_de_sort * 20
        ...
        if crit_esquive >=95:
            ...
            self.hp_basic -= ((1-(magic_resistance/500)) * (magic_dammage *niveau_de_sort))
            other.mana_basic -= niveau_de_sort * 20
            print(f"vous avez infligé {int(((1-(magic_resistance/500)) * (magic_dammage *niveau_de_sort)))}")
        self.hp_basic = int(self.hp_basic)
